export interface DashboardHeaderProps {
  ...
  setDisplayFriendsList: (value: boolean) => void;
  searchedFriendsList: any[];
  addChatToChats: (item: any) => void;
  user: any;
  isLogoutVisible: boolean;
  setIsLogoutVisible: (value: boolean) => void;
  logout: () => void;
  setIsProfileModalVisible: (val: boolean) => void;
  newGroupModal: boolean;
  setNewGroupModal: (value: boolean) => void;
  searchFriends: (email: string) => Promise<any>;
  chats: any[];
...
const DashboardHeader = ({
  ...
  return (
    <div className="bg-dark text-light p-3 sm:p-4 flex justify-between items-center shadow-md w-full relative">
      ...
      <div
        className={`flex flex-row border border-primary rounded-lg items-center p-2 gap-2 mx-2 sm:mx-4 flex-1 max-w-xs sm:max-w-sm ${
